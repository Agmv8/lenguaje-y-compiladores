# 🌟 Sintaxis Digital

> *“Transformando lenguaje en lógica computacional.”*

---

## 📚 Tópico de investigación – Tema 1

**#1 – Comparativa de AFD vs AFN en el análisis léxico de lenguajes modernos**  
*Autómatas finitos deterministas y no deterministas aplicados al análisis léxico*

---

## 📖 Sobre el proyecto

Este repositorio contiene los entregables del **Tema 1** de la asignatura *Lenguajes y Compiladores* (UNEG – Ing. Félix Márquez).

Investigamos la **comparativa entre Autómatas Finitos Deterministas (AFD) y Autómatas Finitos No Deterministas (AFN)** en el contexto del análisis léxico de lenguajes modernos, incluyendo su equivalencia, transformación y aplicaciones prácticas en herramientas como Flex, RE2 y ANTLR.

